...
function Edit({auth, solicitud, estatus, personal, actividades, periodos, departamentos}) {
   ...
   const personalSelectItems = [];
    personal.map((registro) =>{
      personalSelectItems.push({label: registro.nombre, value: registro.id})
    })
   //Lista de estatus para seleccionar
      ...
      >
        ...
        <div className="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
            <form onSubmit={submit}>
                ...
                <InputLabel forInput={data.no_control}>Calificacion</InputLabel>
                <input
                    value={data.calificaion}
                    onChange={(e) => setData("calificaion", e.target.value)}
                    type="text"
                    autoFocus
                    className="mb-3 block w-full border-gray-300 rounded-lg"
                    name="calificaion"
                ...
                <Link href={route('solicitud.index')} className="mt-4 font-semibold text-xs text-white bg-red-600 hover:bg-red-700 rounded-md mr-2 mb-2 px-4 py-2 uppercase">
                    Cancelar
                </Link>
                <PrimaryButton
